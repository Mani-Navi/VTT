...
import VoiceParticipant from "./VoiceParticipant.jsx";
import { RefreshCw, MicOff } from "lucide-react";

/**
 * مرز خطای اختصاصی پنل صوتی برای جلوگیری از توقف میز بازی در خطاهای WebRTC / LiveKit
 */
class VoiceErrorBoundary extends Component {
    ...
    componentDidCatch(error, errorInfo) {
        if (import.meta.env.DEV) {
            console.warn("[VoicePanel Crash]:", error, errorInfo);
        }
    }
    ...
    render() {
        if (this.state.hasError) {
            return (
                <div className="p-3 bg-zinc-950 border border-zinc-800 rounded-xl text-center space-y-2 text-xs font-fa text-zinc-400">
                    <MicOff className="w-5 h-5 text-amber-500 mx-auto" />
                    <p>خطا در اتصال به سرویس صوت</p>
                    <button
                        type="button"
                        onClick={() => this.setState({ hasError: false })}
                        className="px-2.5 py-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 rounded text-[11px] font-bold flex items-center justify-center gap-1 mx-auto cursor-pointer"
                    >
...
const VoicePanelContent = memo(({ roomId }) => {
    const {
        participants,
        isMicEnabled,
        isConnected,
        ...
        >
          ...
          />
            ...
            {isConnecting ? "در حال اتصال..." : isConnected ? "چت صوتی فعال" : "صدا قطع"}
                ...
                <span className="text-[10px] text-zinc-500 font-mono">{participants.length} نفر</span>
            </div>
                ...
                {participants.length > 0 ? (
                    participants.map((p) => (
                        <VoiceParticipant
                            key={p.identity}
                            participant={p}
                            isSpeaking={Boolean(speakingMap[p.identity] || (p.isLocal && isMicEnabled))}
                        />
                    ))
                ) : (
...
VoicePanelContent.displayName = "VoicePanelContent";

export default function VoicePanel({ roomId }) {
    return (
        <VoiceErrorBoundary>
            <VoicePanelContent roomId={roomId} />
        </VoiceErrorBoundary>
    );
}